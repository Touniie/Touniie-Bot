 module.exports.config = {
	...
	version: "1.0.5",
	hasPermssion: 2,
	credits: "Mirai team, Rii mod nhẹ",
...
module.exports.languages = {
	"vi": {
		...
		"banSuccess": "[ Ban Thread ] - Xử lý thành công yêu cầu cấm nhóm có ID %1",
		"unbanSuccess": "[ Unban Thread ] - Xử lý thành công yêu cầu gỡ cấm nhóm có ID %1",
		"banCommandSuccess": "[ Ban Command Thread ] - Xử lý thành công yêu cầu cấm đối với nhóm có ID %1",
		"unbanCommandSuccess": "[ Unban Command Thread ] - Xử lý thành công yêu cầu gỡ cấm đối với %1 riếng với nhóm có ID %2",
		...
		"existBan": "[ Ban Thread ] - ID %1 hiện vẫn bị ban từ trước %2 %3",
		...
		"notExistBanCommand": "[ Unban Command Thread ] - Hiện tại ID nhóm bạn nhập chưa từng bị cấm sử dụng lệnh ",

		"returnBan": "[ Ban Thread ] - Hiện tại bạn vừa yêu cầu cho nhóm này trở về cát bụi\n❯ ID nhóm cần cấm: %1%2\n\n❮React tin nhắn này nếu xác nhận cấm ❯",
		"returnUnban": "[ Unban Thread ] - Hiện tại bạn vừa yêu cầu cho nhóm này một ân huệ về hòa nhập với xã hội:\n❯ ID nhóm cần gỡ cấm: %1\n\n❮React tin nhắn này để xác nhận gỡ cấm❯",
		"returnBanCommand": "[ Ban Command Thread ] - Hiện tại bạn đang yêu cầu cấm sử dụng lệnh đối với nhóm:\n - ID nhóm cần cấm: %1\n- Các lệnh cần cấm: %2\n\n❮ Reaction tin nhắn này để xác thực ❯",
		"returnUnbanCommand": "[ Unban Command Thread ] - Hiện tại bạn đang yêu cầu gỡ cấm sử dụng lệnh đối với nhóm:\n - ID nhóm cần gỡ cấm lệnh: %1\n- Các lệnh cần gỡ cấm: %2\n\n❮ Reaction tin nhắn này để xác thực ❯",
		...
		"returnInfo": "[ Info Thread ] Đây là một số thông tin về nhóm mà bạn đang tìm:\n- ID của nhóm: %1\nCó bị ban:%3 %4\nBị ban lệnh: %5"
	},
	"en": {
		"reason": "Reason",
		"at": "At",
...
module.exports.run = async ({ event, api, args, Threads, getText }) => { 
	...
	switch (args[0]) {
		...
		case "-i": {
			...
			return api.sendMessage(getText("returnInfo", targetID, ((!dateAdded) ? "yes" : "no"), ((reason) ? `${getText("reason")}: "${reason}"` : ""), ((dateAdded) ? `${getText("at")}: ${dateAdded}` : ""), ((commandBanned) ? `${commandBanned.join(", ")}` : "no")), threadID, messageID);
		}
	}
}