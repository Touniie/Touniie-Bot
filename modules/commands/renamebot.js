...
if (threadError != 0) return api.sendMessage("⚠️ Không thể đổi tên tại " + threadError.lenght + " Nhóm",event.threadID, event.messageID)
        }, event.messageID);
    }
    else {
for (const idThread of allThread) {
...
await new Promise(resolve => setTimeout(resolve, 500));
 ...
 return api.sendMessage(`✅ Đã đổi tên thành công cho ${count} nhóm`, event.threadID, () => {
if (threadError != 0) return api.sendMessage("⚠️ Không thể đổi tên tại " + threadError.length + " Nhóm",event.threadID, event.messageID)
        }, event.messageID);
    }
}