        ...
        switch (logMessageType) {
            case "log:thread-admins": {
                 if (logMessageData.ADMIN_EVENT == "add_admin") {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`[ CẬP NHẬT NHÓM ]\n\n📝 Đã cập nhật người dùng ${await Users.getNameUser(logMessageData.TARGET_ID)} trở thành quản trị viên nhóm`, threadID, async (error, info) => {
                    ...
                    if (global.configModule[this.config.name].sendNoti) api.sendMessage(`[ CẬP NHẬT NHÓM ]\n\n📝 Đã cập nhật người dùng ${await Users.getNameUser(logMessageData.TARGET_ID)} trở thành thành viên`, threadID, async (error, info) => {
                        if (global.configModule[this.config.name].autoUnsend) {
                            await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                            return api.unsendMessage(info.messageID);
                        } else return;
            ...
            case "log:user-nickname": {
                ...
                if (global.configModule[this.config.name].sendNoti) api.sendMessage(`[ CẬP NHẬT NHÓM ]\n\n📝 Đã cập nhật biệt danh của người dùng ${await Users.getNameUser(logMessageData.participant_id)}\n✏️ Thành: ${(logMessageData.nickname.length == 0) ? "tên gốc": logMessageData.nickname}`, threadID, async (error, info) => {
                    if (global.configModule[this.config.name].autoUnsend) {
                        await new Promise(resolve => setTimeout(resolve, global.configModule[this.config.name].timeToUnsend * 1000));
                        return api.unsendMessage(info.messageID);
                    } else return;
